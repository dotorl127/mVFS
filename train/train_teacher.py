...
def face_mask_to_bgr_uint8(m: torch.Tensor) -> np.ndarray:
    x = m.detach().float().cpu()[0].clamp(0, 1).numpy()
...
def make_debug_panel(identity, condition_rgb, landmark_map, face_mask, recon, clean) -> np.ndarray:
    return np.hstack([
        ...
        add_caption(face_mask_to_bgr_uint8(face_mask), 'FACE SEG'),
...
def save_debug_images(out_dir: Path, step: int, epoch: int, batch, recon_images: torch.Tensor, max_samples: int = 1):
    ...
    rows = []
    for i in range(n):
        panel = make_debug_panel(batch['identity'][i], batch['condition_rgb'][i], batch['landmark_map'][i], batch['face_mask'][i], recon_images[i], batch['clean'][i])
...
def build_parser():
    p = argparse.ArgumentParser('Train MVFS teacher with APPLE-style face-seg blur + LPIPS')
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            decode_recon = True if (args.lambda_l1 > 0 or args.lambda_lpips > 0 or id_this_step or need_debug) else False
            ...
            if accum_count >= grad_accum:
                ...
                if args.save_every > 0 and global_step > 0 and global_step % args.save_every == 0:
                    ...
                    print(f'[SAVE] opt_step={global_step}')

            if micro_step % args.log_every == 0:
                pbar.set_postfix_str(
                    f'L={loss.item():.3f} N={loss_noise.item():.3f} '
                    f'L1={loss_l1.item():.3f} P={loss_lpips.item():.3f} '
                    f'I={loss_id.item():.3f} id={int(id_active)} s={global_step} a={accum_count} skip={skipped_nonfinite}'
                )
