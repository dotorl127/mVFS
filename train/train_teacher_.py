...
def save_checkpoint(
    ...
    name = f"teacher_epoch_{epoch:04d}_opt_{opt_step:07d}_micro_{micro_step:07d}.pt"
...
def _move_optimizer_state_to_device(opt, device: str):
...
def load_checkpoint(model: MVFSTeacherSDTurbo, ckpt_path: str | Path, opt=None, scaler=None):
    ...
    if opt is not None and ckpt.get("optimizer") is not None:
        ...
        _move_optimizer_state_to_device(opt, model.device_name)
...
def make_debug_panel(identity, blur_rgb, face_mask, landmark_map, recon, clean) -> np.ndarray:
    return np.hstack([
        add_caption(tensor_to_bgr_uint8(identity), "ID"),
        add_caption(tensor_to_bgr_uint8(blur_rgb), "BLUR RGB -> VAE LATENT"),
...
def save_debug_images(
    ...
    rows = []
    blur_key = "blur_rgb" if "blur_rgb" in batch else "condition_rgb"
    ...
    for i in range(n):
        ...
        cv2.putText(meta, f"micro={step} epoch={epoch} sample={i} person_id={person_id}", (10, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (30, 30, 30), 2, cv2.LINE_AA)
        rows.append(np.vstack([meta, panel]))

    out_path = debug_dir / f"micro_{step:07d}.jpg"
    cv2.imwrite(str(out_path), rows[0] if len(rows) == 1 else np.vstack(rows))
...
def main(args):
    ...
    resume_state = {"epoch": 0, "opt_step": 0, "micro_step": 0, "accum_count": 0}
    if args.resume:
        resume_state = load_checkpoint(model, args.resume, opt=opt, scaler=scaler)
        print(
            f"[RESUME] loaded {args.resume} | "
            f"epoch={resume_state['epoch']} "
            f"opt_step={resume_state['opt_step']} "
            f"micro_step={resume_state['micro_step']} "
            f"accum_count={resume_state['accum_count']}"
        )
    ...
    opt_step = int(resume_state["opt_step"])
    micro_step = int(resume_state["micro_step"])
    accum_count = 0
    skipped_nonfinite = 0
        ...
        for batch in pbar:
            ...
            if not torch.isfinite(loss):
                ...
                if need_debug and out.recon_image is not None:
                    save_debug_images(
                        out_dir, micro_step, epoch, batch, out.recon_image,
                        args.debug_num_samples, keep_last=args.debug_max_save
                    )
                micro_step += 1
                continue
            ...
            if args.save_every_micro > 0 and micro_step > 0 and micro_step % args.save_every_micro == 0:
                save_checkpoint(model, out_dir, opt, scaler, epoch, opt_step, micro_step, accum_count)
                print(f"[SAVE] micro={micro_step}, opt={opt_step}, epoch={epoch}")
    ...
    save_checkpoint(model, out_dir, opt, scaler, args.epochs, opt_step, micro_step, accum_count)
